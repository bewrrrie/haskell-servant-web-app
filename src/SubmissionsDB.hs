...
insertSubmissionStatement :: Statement (T.Text, T.Text, Int16) ()
insertSubmissionStatement =
  let sql =
        "insert into submissions (name, text, score) \
        \values ($1, $2, $3)"
      encoder =
        contrazip3
...
addSubmission conn name txt score =
  runSession conn $ statement (name, txt, score) insertSubmissionStatement
